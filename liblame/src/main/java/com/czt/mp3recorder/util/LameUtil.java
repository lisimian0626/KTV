...
public class LameUtil {
	 ...
	 *
	 *
	 * @param flag 1 single channel sound 2 left and rigth sound
	 *
	...
	public native static int encode(short[] bufferLeft, short[] bufferRight,
			int samples, byte[] mp3buf, int flag);

	/**
	 *
	 * @param buffer pcm buff
	 * @param samples pcm buffere len
	 * @param mp3buf result encoded MP3 stream. You must specified
	 * @param flag 1 single channel sound 2 left and rigth sound
	 * @return
	 */
	public native static int encode2(byte[] buffer,
									int samples, byte[] mp3buf, int flag);
	/**
	 * Flush LAME buffer.
	 * 
	...
	public native static int writeXingFrame(byte[] mp3buf);
