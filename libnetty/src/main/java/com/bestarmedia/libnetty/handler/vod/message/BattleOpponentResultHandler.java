 ...
 * 对方斗歌结果(12880022)
 */
public class BattleOpponentResultHandler implements ProtoHandlerInterface<ProtoFrame> {

    @Override
    public void handle(Channel channel, ProtoFrame protoFrame) {
        try {
            BattleContract msg = BattleContract.parseFrom(protoFrame.byteBuf);
            Log.i(NettyConfig.TAG, "对方斗歌结果id:" + msg.getId());
            EventBusUtil.postSticky(EventBusId.ImId.BATTLE_OPPONENT_RESULT, msg);
        } catch (Exception e) {
