...
import com.beidousat.karaoke.widget.viewpager.WidgetGiftPager;
...
import com.bestarmedia.libcommon.model.erp.FreeCategoriesV4;
 ...
 */
public class FmGifts extends BaseFragment implements OnPageScrollListener, View.OnClickListener, AdtShopType.OnShopTypeClickListener, WidgetPage.OnPageChangedListener {

    private View mRootView;
    private WidgetGiftPager mPager;
    ...
    public static FmGifts newInstance(String account, String password) {
        FmGifts fragment = new FmGifts();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        mRootView = inflater.inflate(R.layout.fm_gifts, null);
        ...
        HorizontalDividerItemDecoration horDivider = new HorizontalDividerItemDecoration.Builder(getContext())
                .color(Color.TRANSPARENT).size(8).margin(8)
                .build();
    ...
    public void onSuccess(String method, Object object) {
        ...
        if (isAdded()) {
            if (RequestMethod.NODE_PURCHASE_FREE.equalsIgnoreCase(method)) {
                FreeCategoriesV4 freeCategoriesV4;
                if (object instanceof FreeCategoriesV4 && (freeCategoriesV4 = (FreeCategoriesV4) object).free != null
                        && freeCategoriesV4.free.data != null && freeCategoriesV4.free.data.size() > 0) {
                    Logger.d(getClass().getSimpleName(), "category:" + freeCategoriesV4.free.data.get(0).Name);
                    mAdtShopType.setData(freeCategoriesV4.free.data);
                    mAdtShopType.notifyDataSetChanged();
                    requestGoods(freeCategoriesV4.free.data.get(0).CateID);
                }
            } else if (RequestMethod.NODE_PURCHASE.equalsIgnoreCase(method)) {
                Purchases purchases;
                if (object instanceof Purchases && (purchases = (Purchases) object).purchase != null && purchases.purchase.size() > 0) {
    ...
    private void requestCategory() {
        HttpRequestV4 request = initRequestV4(RequestMethod.NODE_PURCHASE_FREE);
        ...
        request.addParam("verify_mode", String.valueOf(1));
        request.setConvert2Class(FreeCategoriesV4.class);
        request.get();
    }
    ...
    private void requestGoods(String cateId) {
        ...
        request.addParam("room_code", VodConfigData.getInstance().getRoomCode());
        request.addParam("type", String.valueOf(1));
    ...
    private void initShopPager(List<Good> goods) {
        int mTotalPage = mPager.initPager(mCateID, goods);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.tv_shopping_car) {
            ...
            if (giftData.getGoodList() != null && giftData.getGoodList().size() > 0)
                FragmentUtil.addFragment(FmGiftCart.newInstance(mAccount, mPassword), false, false, true, false);
        }
    }
}
