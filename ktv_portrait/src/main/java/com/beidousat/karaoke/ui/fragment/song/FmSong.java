        ...
        if (isAdded()) {
            if (RequestMethod.V4.VOD_NEW_SONG.equalsIgnoreCase(method)) {
                if (object instanceof NewSong) {
                    ...
                    if (newSongInfo != null) {
                        Glide.with(getContext()).load(TextUtils.isEmpty(newSong.newSongInfo.imageUrl) ? newSong.newSongInfo.image : newSong.newSongInfo.imageUrl).error(R.drawable.song_new).into(im_new_song);
                    } else {
