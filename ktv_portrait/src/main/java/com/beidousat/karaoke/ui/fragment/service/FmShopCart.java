...
import com.beidousat.karaoke.widget.viewpager.WidgetShopCartPage;
 ...
 */
public class FmShopCart extends BaseFragment implements View.OnClickListener, AdtShopCart.OnShopCartPriceChangedListener {


    private View mRootView;
    private WidgetShopCartPage mPager;
    private TextView mTvTotal;
    private Button mBtnPay, mBtnDel;
    private String mAccount, mPassword;
    private boolean mVerifyPwd = true;

    public static FmShopCart newInstance(String account, String password, boolean verifyPwd) {
        FmShopCart fragment = new FmShopCart();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        mRootView = inflater.inflate(R.layout.fm_shopping_cart, null);
        ...
        mBtnDel = mRootView.findViewById(R.id.btn_call_service);
    ...
    private void checkAccountPwdAndSubmit() {
        if (VodConfigData.getInstance().verifyPasswordOrderWine()) {//重新验证
            DlgAccountPassword orderSubmit = new DlgAccountPassword(getActivity(), 0);
            orderSubmit.setOnAccountPwdListener((account, pwd, isVerifyPwd) ->
                    submitOrder(account, pwd, isVerifyPwd));
            orderSubmit.show();
        } else {//不要重新验证
            if (!TextUtils.isEmpty(mAccount)) {
                submitOrder(mAccount, mPassword, mVerifyPwd);
            } else {
                DlgAccountPassword orderSubmit = new DlgAccountPassword(getActivity(), 0);
                orderSubmit.setOnAccountPwdListener((account, pwd, isVerifyPwd) ->
                        submitOrder(account, pwd, isVerifyPwd));
    ...
    private ShopCart getNoSelectGiftGood() {
        List<ShopCart> shopCarts = new ShopCartData().getGoodList();
...
//                        || cart.comboGoods == null || cart.comboGoods.size() <= 0
    ...
    private void submitOrder(final String account, final String pwd, final boolean isVerifyPwd) {
