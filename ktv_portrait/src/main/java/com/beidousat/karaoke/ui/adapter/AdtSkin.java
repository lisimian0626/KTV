...
public class AdtSkin extends RecyclerView.Adapter<AdtSkin.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, final int i) {
        ...
        if (TextUtils.isEmpty(skinInfo.id)) {//默认皮肤
            Glide.with(mContext).load(R.drawable.skin_default_v).skipMemoryCache(false).into(viewHolder.ivCover);
        } else {
            Glide.with(mContext).load(TextUtils.isEmpty(skinInfo.verticalImgUrl) ? skinInfo.verticalImg : skinInfo.verticalImgUrl).into(viewHolder.ivCover);
        }
