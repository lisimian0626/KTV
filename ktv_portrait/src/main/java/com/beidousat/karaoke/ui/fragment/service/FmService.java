        ...
        switch (view.getId()) {
            case R.id.wm_bill:
                if (NodeRoomInfo.getInstance().isPurchaseEnabled()) {
                    FragmentUtil.addFragment(new FmBill(), false, false, true, false);
                } else {
                ...
                if (NodeRoomInfo.getInstance().isPurchaseEnabled()) {
                    if (VodConfigData.getInstance().verifyPasswordEnterWine()) {
                        ...
                        orderSubmit.setOnAccountPwdListener((account, pwd, isVerifyPwd) ->
                                FragmentUtil.addFragment(FmShop.newInstance(account, pwd, isVerifyPwd), false, false, true, false));
                        orderSubmit.show();
                    } else {
                        FragmentUtil.addFragment(FmShop.newInstance(null, null, true), false, false, true, false);
                    }
                } else {
                    tipMessage(R.string.is_purchase_not_enabled);
                }
            ...
            case R.id.wm_give:
                if (VodConfigData.getInstance().hasGift()) {
                    if (NodeRoomInfo.getInstance().isPurchaseEnabled()) {
                        DlgAccountPassword dlgAccountPassword = new DlgAccountPassword(getContext(), 1);
                        dlgAccountPassword.setOnAccountPwdListener((account, pwd, isVerifyPwd) ->
                                FragmentUtil.addFragment(FmGifts.newInstance(account, pwd), false, false, true, false));
            ...
            case R.id.wm_pr://公关
                if (VodConfigData.getInstance().haveSlotCard()) {
                    if (NodeRoomInfo.getInstance().isPurchaseEnabled()) {
                        FragmentUtil.addFragment(new FmRelationsCard(), false, false, true, false);
                    } else {
                        tipMessage(R.string.is_purchase_not_enabled);
                    }
                } else {
