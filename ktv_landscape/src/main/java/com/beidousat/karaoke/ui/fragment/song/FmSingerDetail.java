 ...
 */
public class FmSingerDetail extends BaseFragment implements OnPageScrollListener, OnPreviewSongListener, KeyboardListener, View.OnClickListener, OnSongSelectListener, WidgetPage.OnPageChangedListener {
    ...
    private List<Language> languageList_list;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mTvSingerName = mRootView.findViewById(R.id.tv_singer_name);
        ...
        mWidgetTopTabs.setLeftTabClickListener(position -> {
            if (languageList_list != null && languageList_list.size() > position) {
                currentLanguage = languageList_list.get(position);
                requestSingerSong();
            }
        });
        ...
        mTvName = mRootView.findViewById(R.id.tv_singer);
            ...
            if (RequestMethod.V4.V4_API_SONG.equalsIgnoreCase(method)) {
                ...
                if (object instanceof LanguageList) {
                    ...
                    if (languageList.languages != null && languageList.languages.size() > 0) {
                        languageList_list = languageList.languages;
                        String[] text = new String[languageList_list.size()];
                        for (int i = 0; i < text.length; i++) {
                            text[i] = languageList_list.get(i).languageName;
                        }
                        mWidgetTopTabs.setLeftTabs(text);
                        currentLanguage = languageList_list.get(0);
                    }
                }
            }
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.tv_search_song:
                Animation mAnimImg = AnimationUtils.loadAnimation(getContext(), R.anim.amin_singer_img_out);
                Animation mAnimContent = AnimationUtils.loadAnimation(getContext(), R.anim.amin_singer_song_tran);
                Animation mAnimKeyboard = AnimationUtils.loadAnimation(getContext(), R.anim.amin_singer_keyboard_in);

                mRootView.findViewById(R.id.tv_search_song).setVisibility(View.GONE);
                mRootView.findViewById(R.id.ll_signer).startAnimation(mAnimImg);
                mRootView.findViewById(R.id.rl_content).startAnimation(mAnimContent);
                ...
                mWidgetKeyboard.startAnimation(mAnimKeyboard);
                break;
            case R.id.tv_singer_name:
                Animation animImgIn = AnimationUtils.loadAnimation(getContext(), R.anim.amin_singer_img_in);
                Animation animSongBack = AnimationUtils.loadAnimation(getContext(), R.anim.amin_singer_song_tran_back);
                Animation animKeyboardOut = AnimationUtils.loadAnimation(getContext(), R.anim.amin_singer_keyboard_out);

                mRootView.findViewById(R.id.ll_signer).startAnimation(animImgIn);
                mRootView.findViewById(R.id.rl_content).startAnimation(animSongBack);
                mWidgetKeyboard.startAnimation(animKeyboardOut);
